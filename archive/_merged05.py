# This script downloads audio from YouTube to a .wav file and grabs metadata too. 
# Then creates it creates a transcript with punctuation and diarization and metadata in json format.

# The following command is an example of how to call this script
# python3 _merged05.py https://www.youtube.com/watch?v=4PKtBhYc_8I

# This script can also be called repeatedly by _process_channel_videos02.py to make transcripts of all videos for a YouTube channel.
...
parser = argparse.ArgumentParser(description="Process a YouTube video: download audio, transcribe, and save as JSON and segmented .txt.")
...
# Step 11: Generate segmented .txt file
txt_output = f"{video_id}.txt"
try:
    # Build metadata line
    metadata_str = (
        f"Metadata: language={whisper_data['language']}, "
        f"platform=youtube.com, "
        f"channelName={metadata['channelName']}, "
        f"videoTitle={metadata['videoTitle']}, "
        f"url={metadata['url']}, "
        f"videoPostDate={metadata['videoPostDate']}"
    )
    ...
    txt_content = [metadata_str] + segments_lines
